...
def define_Cheby_lengths(r_coeffs,z_coeffs,n):
    ...
    L_list = [0 for i in range(no_s)]
    V_list = [0 for i in range(no_s)]
    for i in range(no_s):
        L_list[i] = ((2*func_output(n,r_coeffs,T_func_prime,s_list[i]))**2+(2*func_output(n,z_coeffs,T_func_prime,s_list[i]))**2)**(1/2)
        V_list[i] = (func_output(n,r_coeffs,T_functions,s_list[i]))**2*(2*func_output(n,z_coeffs,T_func_prime,s_list[i]))

    Arclength = scipy.integrate.trapz(L_list,s_list)
    Volume = abs(3.14*scipy.integrate.trapz(V_list,s_list))

    return Arclength,Volume
